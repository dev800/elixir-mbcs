defmodule Mbcs do
  @moduledoc """
  Wrapper for erlang-mbcs_erl.
  This module provides functions for character encoding conversion.
  See `https://code.google.com/p/erlang-mbcs_erl/` for detail.
  ...
  * bom: `true`, `false`
  ...
  def start do
    :mbcs_erl_server.start()
  end

  def stop do
    :mbcs_erl_server.stop()
  end
  ...
  def encode(string, to, options) when is_list(string) do
    case :mbcs_erl.encode(string, to, options) do
  ...
  def decode(string, from, options \\ []) do
    case :mbcs_erl.decode(string, from, options) do
      {:error, reason} ->
        {:error, reason}
  ...
  def from_to(string, from, to, options \\ []) do
    case :mbcs_erl.from_to(string, from, to, options) do
      {:error, reason} -> {:error, reason}
      result -> {:ok, result}
    end
  end
